/* 
Task: Implement a simple GPU reduction kernel that uses only global memory and atomicAdd.
Each thread adds one input element into a single global output variable
*/
